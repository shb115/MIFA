#!/usr/bin/env python3
"""trail_val_check_against_committed.py — DDT-validate every BAKSHEESH
trail the committed attack driver actually consumes at runtime, i.e.
every inner trail in
...
BAKSHEESH S-box.  Prints per-source OK/FAIL summaries and exits with
rc=1 iff any trail fails.

Unlike the sibling `trail_val_check.py` (which validates three hand-
picked example trails), this script reads the exact trails the BAKSHEESH
attack driver consumes at runtime, closing the gap between "we have a
DDT validator" and "the validator actually audited the trails that
run."  No Gurobi required — uses only the S-box / bit-perm tables.

A cross-cipher equivalent that also covers DEFAULT simple and rotating
lives at
    DEFAULT/Codes/find_diff_trail_with_single_sol/trail_check/
...
# Same 128-bit bit-permutation as DEFAULT; see
...
def repo_root():
    # script lives at BAKSHEESH/Codes/find_diff_trail_with_single_sol/trail_check/
...
def main():
    ...
    print("====================================================================")
    print(" DDT validation of EVERY committed BAKSHEESH trail consumed")
    print(" by the attack driver")
    print("====================================================================")
    ...
    print()
    if total_fails == 0:
        print("SUCCESS: every committed BAKSHEESH trail is DDT-consistent.")
        return 0
