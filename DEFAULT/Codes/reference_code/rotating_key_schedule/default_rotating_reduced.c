...
int main(int argc, char **argv)
{
    ...
    // Seed: argv[1] if given, else time(NULL).  Mirror the sibling
    // simple_key_schedule/default_reduced.c convention so a reviewer
    // can reproduce a specific output deterministically.
    unsigned int seed = (argc >= 2) ? (unsigned int)strtoul(argv[1], NULL, 10)
