...
int main(int argc, char **argv)
{
    ...
    if (argc < 2) {
        fprintf(stderr, "usage: %s <round-count> [<seed>]\n", argv[0]);
        fprintf(stderr, "       e.g. %s 6            (uses time(NULL) seed)\n", argv[0]);
        fprintf(stderr, "            %s 6 20260422   (deterministic seed)\n", argv[0]);
        return 1;
    }
    ...
    // Initialize random seed: argv[2] wins if given, else time(NULL)
    unsigned int seed = (argc >= 3) ? (unsigned int)strtoul(argv[2], NULL, 10) : (unsigned int)time(NULL);
    srand(seed);
