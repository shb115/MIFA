"""trail_val_check.py — DDT-level sanity check of hard-coded 6/7/8-round trails against the
DEFAULT S-box (no Gurobi required).

This is a **standalone demonstration**: the three hard-coded trail
sequences below are example single-solution trails used during MIFA
development, NOT the specific trails consumed by the attack drivers in
`Codes/key_recovery_attack/` or enumerated by `trail_search_on_default.py`.
A green "All S-box transitions valid" row here therefore demonstrates that
the DDT-check procedure works on representative trails; it does NOT
validate the specific trails the attacks actually use (those are
validated end-to-end by the attack's success criterion itself —
`mk in r*_keyspace` / `nk* == nk[*] : True`)."""
