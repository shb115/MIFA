"""0x1_2-5r.py — Round-by-round analysis of DEFAULT S-box + bit-perm under input
difference pattern p = 0x1.  Prints the expected non-LS active
nibble count for rounds 5, 4, 3, 2 (seen from the ciphertext end)."""

# NOTE on round naming: variables `r5, r4, r3, r2` refer to the state
# at a given *distance from the ciphertext* in a 6-round cipher.
# Iteration order is forward from the plaintext-side input difference:
#   r5 = state after 1 forward round  (= 5 rounds from ciphertext-end)
#   r4 = state after 2 forward rounds (= 4 rounds from ciphertext-end)
#   ... and so on down to r2.
# The labels mirror the convention used in the paper's Sec. 3.2
# tables; they are NOT 0-indexed forward-round counters.
#
# NOTE on "LS set": {0, 6, 9, 0xf} is the set of DEFAULT S-box input
# differences whose DDT row has a single non-zero column
# (DDT[0][0] = DDT[6][a] = DDT[9][f] = DDT[f][5] = 16) — i.e.
# deterministic-transition input differences.  NOT forward fixed
# points of the S-box.
...
def calculate_expected_other_active_nibbles(round_results, round_name):
    ...
    print(f"--- {round_name} ---")
    excluded_hex = "{" + ", ".join(f"{x:x}" for x in sorted(excluded_group)) + "}"
    print(f"  Expected number of active nibbles NOT in '{excluded_hex}': {expected_count:.8f}\n")
