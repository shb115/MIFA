"""0x2_3-5r.py — Round-by-round analysis of DEFAULT under input difference p = 0x2.
Prints the expected non-LS active nibble count for rounds 3, 4, 5."""

# NOTE on round naming: variables `r5, r4, r3` refer to the state
...
# See the sibling `0x1_2-5r.py` for the full convention, and for the
# definition of the "LS set" {0, 6, 9, 0xf} as the DDT's single-column
# rows (not forward fixed points).
from itertools import product

# --- Start of same function definitions ---
# Given S-box
...
# --- End of same function definitions ---
...
def calculate_expected_other_active_nibbles(round_results, round_name):
    ...
    """
    # Excluded group (0 is not an active nibble, so it is implicitly excluded)
...
# --- Main Analysis Logic for p=0x2 ---
print("### Starting Round-by-Round Analysis for p=0x2 ###\n")
