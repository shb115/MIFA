"""all_diff_3r.py — Exhaustive 3-round non-LS nibble analysis over all 128 single-bit
input differences for DEFAULT; ranks each p by expected non-LS sum.
Runs under a 30 GB memory cap and skips p values that would exceed
that cap.

NOTE on "LS set": here the LS ("linear structure") set is
{0, 6, 9, 0xf} — the set of input-difference nibble values whose
DDT row has a single non-zero column (DDT[0][0] = DDT[6][a] =
DDT[9][f] = DDT[f][5] = 16 for the DEFAULT S-box).  These are
deterministic-transition input differences.  The set is NOT the set
of forward-S-box fixed points — e.g. `sbox[6] == 0xa`, not `6`.
BAKSHEESH's analogous LS set is {0, 8} (see
BAKSHEESH/Codes/count_non_LS/monte_diff.py)."""
...
# --- Settings ---
# The memory cap below is a *process-level* RSS check (Python
# interpreter + all live objects, not just the current round's output
# table).  On a 32 GB host the cap effectively trips when the round
# table alone is ~28 GB, because the interpreter and a few MB of
# overhead also count.  See DEFAULT Sec. 3 in README.md for hardware
# sizing guidance.
MEMORY_LIMIT_GB = 30
...
# A machine-readable marker line comes first so reproduce.sh (and any
# CI / diff tooling) can grep for '# SKIPPED_P_VALUES=N' without
# parsing the human-readable banner.  N == 0 on ≥ 32 GB hosts.
print(f"\n# SKIPPED_P_VALUES={len(skipped_p_values)}")
if skipped_p_values:
    print("### p-values skipped due to memory limit (30GB) ###")
    for p_val in skipped_p_values:
        print(f"  - 0x{p_val:x}")
else:
    print("All p-values analyzed successfully (No memory limit exceeded)")
